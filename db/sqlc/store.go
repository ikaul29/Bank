...
// SQL store provides all functions to execute db queries and transactions
...
// input params for transfer transaction
...
func (store *SQLStore) TransferTx(ctx context.Context, arg TransferTxParams) (TransferTxResult, error) {
	...
	err := store.execTx(ctx, func(q *Queries) error {
        ...
        // this is make sure deadlock doesn't occur,
		// All Tx update accounts in the same order ie, smaller accountId gets exec first
		// whenever Tx is updating an account balance, all other Tx(goRoutines) will be blocked till Tx has been committed
		// query order matters
